...
async function addtocart(game_id, image) {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      alert("Game not found in database! Check the id.");
      return;
    }
  ...
  try {
    const postRes = await fetch(`${SUPABASE_URL}/rest/v1/shopping_cart_table`, {
      method: 'POST',
      headers: {
    ...
    if (postRes.ok) {
      console.log(" Saved to Supabase cart");
    } else {
